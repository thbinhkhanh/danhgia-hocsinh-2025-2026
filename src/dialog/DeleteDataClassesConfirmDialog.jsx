...
export default function DeleteDataClassesConfirmDialog({
  ...
  const localClasses = React.useMemo(
    () => selectedClasses ?? [],
    [selectedClasses]
  );
  return (
      ...
      <DialogContent sx={{ px: 3, py: 3 }}>
        <Box sx={{ textAlign: "center", py: 1 }}>
          <Typography sx={{ fontSize: 14, color: "#334155" }}>
            {selectedCount === 1 ? (
              <>
                Bạn có chắc chắn muốn xóa dữ liệu kiểm tra lớp{" "}
                <b>{selectedClasses?.[0] ?? ""}</b>
              </>
            ) : (
              <>
                Bạn có chắc chắn muốn xóa dữ liệu kiểm tra{" "}
          ...
          >
            {/*Thao tác này sẽ xóa toàn bộ dữ liệu bài làm của các lớp đã chọn. <br />*/}
      ...
      >
        <Stack direction="row" spacing={2}>
          ...
          <Button
            onClick={async () => {
              try {
                await onConfirmDelete?.(localClasses);
              } catch (err) {
                console.error("❌ onConfirmDelete crashed:", err);
              }
            }}
            variant="contained"
            sx={{
