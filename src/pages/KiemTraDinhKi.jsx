...
const handleSaveAll = async () => {
  ...
  const handleDownload = async () => {
    try {
      await exportEvaluationToExcelFromTable(students, selectedClass);
    } catch (error) {
...
const nhanXetTheoMuc = {
  ...
  <Box sx={{ minHeight: "100vh", backgroundColor: "#e3f2fd", pt: 3 }}>
    ...
    >
      ...
      {/* 🟦 Ô chọn học kỳ ở góc trên bên phải */}
      <Box
        sx={{
          position: "absolute",
          top: 12,
      ...
      {/* 🟩 Chọn lớp và checkbox Công nghệ */}
      <Stack
      ...
      >
        ...
        <FormControlLabel
          control={
            <Checkbox
              checked={isCongNghe}
              onChange={(e) => setIsCongNghe(e.target.checked)}
            />
          }
          label="Công nghệ"
        />
