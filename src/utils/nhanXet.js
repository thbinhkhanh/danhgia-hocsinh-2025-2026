// nhanXet.js
//export const nhanXetTinHoc = {
export const nhanXetTinHocGiuaKy = {
  tot: [
    "Em có ý thức học tập tốt, thao tác thành thạo và tích cực trong thực hành Tin học.",
    "Em chủ động, tự tin, biết vận dụng CNTT vào học tập và đời sống.",
    "Em học tập nghiêm túc, thao tác nhanh, nắm vững kiến thức Tin học cơ bản.",
    "Em thể hiện kỹ năng sử dụng máy tính thành thạo, làm việc khoa học và hiệu quả.",
    "Em yêu thích môn Tin học, chủ động khám phá trong học tập.",
    "Em có khả năng vận dụng kiến thức vào giải quyết tình huống thực tế liên quan đến CNTT.",
    "Em thao tác nhanh, chính xác, sử dụng phần mềm đúng quy trình và sáng tạo.",
    "Em có tư duy logic tốt, biết trình bày và lưu trữ sản phẩm học tập khoa học.",
    "Em tiếp thu nhanh, thực hành thuần thục, hoàn thành tốt các nhiệm vụ học tập.",
    "Em thể hiện tinh thần hợp tác, chia sẻ và giúp đỡ bạn trong hoạt động nhóm."
  ],
  kha: [
    "Em có ý thức học tập tốt, biết sử dụng thiết bị và phần mềm cơ bản.",
    "Em tiếp thu bài khá, cần chủ động hơn trong việc thực hành và vận dụng kiến thức.",
    "Em làm bài cẩn thận, có tinh thần học hỏi nhưng cần rèn luyện thêm thao tác thực hành.",
    "Em nắm được kiến thức trọng tâm, thực hiện thao tác tương đối chính xác.",
    "Em có khả năng sử dụng máy tính ở mức khá, cần luyện tập thêm để tăng tốc độ thao tác.",
    "Em có tinh thần học tập tích cực nhưng đôi khi còn thiếu tự tin khi thực hành.",
    "Em đã biết áp dụng kiến thức để tạo sản phẩm học tập, cần sáng tạo hơn trong trình bày.",
    "Em có tiến bộ rõ, cần phát huy thêm tính chủ động trong học tập Tin học.",
    "Em biết hợp tác trong nhóm, hoàn thành nhiệm vụ được giao tương đối tốt.",
    "Em thực hành đúng hướng dẫn, cần nâng cao hơn khả năng vận dụng vào tình huống mới."
  ],
  trungbinh: [
    "Em hoàn thành các yêu cầu cơ bản, cần cố gắng hơn khi thực hành.",
    "Em còn lúng túng trong thao tác, cần sự hỗ trợ thêm từ giáo viên.",
    "Em có tiến bộ nhưng cần rèn luyện thêm kỹ năng sử dụng phần mềm.",
    "Em hiểu bài nhưng thao tác chậm, cần rèn luyện thêm để nâng cao hiệu quả.",
    "Em đôi khi còn quên thao tác cơ bản, cần ôn tập thường xuyên hơn.",
    "Em hoàn thành nhiệm vụ học tập, cần chủ động hơn trong học tập.",
    "Em có thái độ học tập đúng đắn nhưng cần tập trung hơn khi làm việc với máy tính.",
    "Em nắm được một phần kiến thức, cần hỗ trợ thêm để vận dụng chính xác.",
    "Em có cố gắng, tuy nhiên còn gặp khó khăn khi làm bài thực hành.",
    "Em cần tăng cường luyện tập để cải thiện kỹ năng thực hành.",

    "Em chưa nắm chắc kiến thức, thao tác còn chậm, cần được hướng dẫn nhiều hơn.",
    "Em cần cố gắng hơn trong học tập, đặc biệt là phần thực hành Tin học.",
    "Em còn gặp nhiều khó khăn khi sử dụng phần mềm, cần được hỗ trợ thường xuyên.",
    "Em chưa chủ động trong học tập, cần khuyến khích và theo dõi thêm.",
    "Em thao tác thiếu chính xác, cần rèn luyện thêm kỹ năng cơ bản.",
    "Em tiếp thu chậm, cần sự kèm cặp sát sao để tiến bộ hơn.",
    "Em cần dành nhiều thời gian hơn cho việc luyện tập trên máy tính.",
    "Em chưa hoàn thành được yêu cầu bài học, cần hỗ trợ từ giáo viên và bạn bè.",
    "Em cần được củng cố lại kiến thức nền tảng và hướng dẫn thực hành cụ thể hơn."
  ],
  yeu: [
    "Em chưa nắm chắc kiến thức, thao tác còn chậm, cần được hướng dẫn nhiều hơn.",
    "Em cần cố gắng hơn trong học tập, đặc biệt là phần thực hành Tin học.",
    "Em còn gặp nhiều khó khăn khi sử dụng phần mềm, cần được hỗ trợ thường xuyên.",
    "Em chưa chủ động trong học tập, cần khuyến khích và theo dõi thêm.",
    "Em thao tác thiếu chính xác, cần rèn luyện thêm kỹ năng cơ bản.",
    "Em tiếp thu chậm, cần sự kèm cặp sát sao để tiến bộ hơn.",
    "Em cần dành nhiều thời gian hơn cho việc luyện tập trên máy tính.",
    "Em chưa hoàn thành được yêu cầu bài học, cần hỗ trợ từ giáo viên và bạn bè.",
    "Em cần được củng cố lại kiến thức nền tảng và hướng dẫn thực hành cụ thể hơn."
  ]
};

export const nhanXetTinHocCuoiKy = {
  tot: {
    lyThuyet: [
      "Em nắm vững kiến thức Tin học",
      "Em hiểu sâu, trình bày rõ ràng",
      "Em chủ động tham gia xây dựng",
      "Em vận dụng tốt kiến thức",
      "Em có tư duy logic tốt",
      "Em ghi nhớ, áp dụng kiến thức",
      "Em liên hệ kiến thức thực tế",
      "Em tự tin trả lời câu hỏi",
      "Em học tập nghiêm túc, tiếp thu",
      "Em có nền tảng kiến thức vững"
    ],
    thucHanh: [
      "thao tác thành thạo, nhanh, chính xác.",
      "sử dụng phần mềm đúng quy trình.",
      "hoàn thành tốt các bài thực hành.",
      "tạo sản phẩm học tập chất lượng.",
      "vận dụng kiến thức thực hành tốt.",
      "thao tác tự tin, ít sai sót.",
      "thực hành sáng tạo, có cải tiến.",
      "dùng máy tính an toàn, đúng quy."
    ]
  },

  kha: {
    lyThuyet: [
      "Em nắm được kiến thức cơ bản",
      "Em hiểu bài nhưng cần tự tin",
      "Em tiếp thu bài khá",
      "Em có tiến bộ trong học tập",
      "Em cần luyện tập để ghi nhớ",
      "Em trình bày bài tương đối rõ",
      "Em tham gia xây dựng chưa đều",
      "Em cần chủ động hơn trong học"
    ],
    thucHanh: [
      "thực hành đúng hướng dẫn.",
      "thao tác cơ bản tốt.",
      "hoàn thành sản phẩm đúng yêu cầu.",
      "còn chậm nhưng đảm bảo chính xác.",
      "cần luyện tập nâng cao kỹ năng.",
      "sử dụng được phần mềm đã học.",
      "cần tăng tốc độ khi thực hành."
    ]
  },

  trungbinh: {
    lyThuyet: [
      "Em nắm được một phần kiến thức",
      "Em cần ôn tập để hiểu bài",
      "Em tiếp thu bài còn chậm",
      "Em chưa tự tin khi trình bày",
      "Em cần hỗ trợ thêm trong học",
      "Em ghi nhớ kiến thức chưa bền",
      "Em cần chủ động hơn trong giờ"
    ],
    thucHanh: [
      "thao tác còn chậ.m",
      "thực hành chưa linh hoạt.",
      "hoàn thành nhiệm vụ mức cơ bản.",
      "còn lúng túng khi dùng phần mềm.",
      "cần luyện tập thường xuyên hơn.",
      "thao tác còn sai sót.",
      "cần được hướng dẫn thêm."
    ]
  },

  yeu: {
    lyThuyet: [
      "Em chưa nắm chắc kiến thức Tin",
      "Em gặp khó khăn khi ghi nhớ",
      "Em cần cố gắng hơn trong học",
      "Em tiếp thu bài chậm",
      "Em thiếu tự tin khi học lý"
    ],
    thucHanh: [
      "thao tác chưa chính xác",
      "gặp nhiều khó khăn khi thực hành.",
      "chưa hoàn thành bài thực hành.",
      "cần được hướng dẫn chi tiết hơn.",
      "cần luyện tập nhiều hơn.",
      "chưa sử dụng thành thạo máy tính."
    ]
  }
};


// Giữa kỳ
export const nhanXetCongNgheGiuaKy = {
  tot: [
    "Em học tập tích cực, cẩn thận và hoàn thành tốt các sản phẩm Công nghệ.",
    "Em có ý thức học tập tốt, sáng tạo trong các hoạt động thực hành.",
    "Em làm việc chăm chỉ, biết giữ gìn dụng cụ và đảm bảo an toàn khi thực hành.",
    "Em chủ động, tự tin và hợp tác tốt với bạn trong các hoạt động nhóm.",
    "Em yêu thích môn Công nghệ, biết vận dụng kiến thức vào cuộc sống hằng ngày.",
    "Em có nhiều ý tưởng sáng tạo, sản phẩm thực hành đẹp và đúng yêu cầu.",
    "Em làm việc ngăn nắp, khoa học và luôn cố gắng hoàn thành tốt nhiệm vụ.",
    "Em học tập tích cực, sáng tạo trong các hoạt động thực hành.",
    "Em hứng thú trong giờ học, thực hiện đúng hướng dẫn của thầy cô.",
    "Em thể hiện sự khéo léo, cẩn thận và tỉ mỉ khi làm sản phẩm."
  ],
  kha: [
    "Em có ý thức học tập tốt, hoàn thành các nhiệm vụ được giao.",
    "Em thực hành đúng hướng dẫn, sản phẩm đạt yêu cầu cơ bản.",
    "Em chăm chỉ, có tiến bộ trong học tập môn Công nghệ.",
    "Em học tập nghiêm túc, cần mạnh dạn hơn khi trình bày ý tưởng.",
    "Em thực hiện được các thao tác cơ bản, cần rèn luyện thêm sự khéo léo.",
    "Em học tập nghiêm túc, cần phát huy thêm tính sáng tạo.",
    "Em biết sử dụng dụng cụ đúng cách, cần tập trung hơn khi làm việc.",
    "Em có tinh thần học hỏi, cần chủ động hơn trong các hoạt động thực hành.",
    "Em làm việc cẩn thận, sản phẩm tương đối tốt, cần cố gắng thêm.",
    "Em có ý thức tốt trong giờ học, cần rèn thêm kỹ năng thực hành."
  ],
  trungbinh: [
    "Em hoàn thành nhiệm vụ học tập ở mức cơ bản, cần cố gắng hơn.",
    "Em còn chậm trong khi thực hành, cần tập trung và cẩn thận hơn.",
    "Em có tiến bộ nhưng thao tác còn chưa chính xác.",
    "Em cần chủ động hơn khi làm việc và trao đổi với bạn bè.",
    "Em thực hành đúng hướng dẫn nhưng sản phẩm chưa đẹp.",
    "Em có thái độ học tập đúng, cần luyện tập thêm để thành thạo hơn.",
    "Em còn chậm trong khi thực hành, cần cố gắng hơn.",
    "Em làm việc còn vội vàng, cần rèn tính kiên trì và tỉ mỉ.",
    "Em có cố gắng nhưng cần được hướng dẫn thêm trong giờ học.",
    "Em cần luyện tập thêm để nâng cao kỹ năng thực hành.",

    "Em cần cố gắng hơn trong học tập và rèn luyện kỹ năng thực hành.",
    "Em còn lúng túng khi thực hành, cần sự giúp đỡ của thầy cô và bạn bè.",
    "Em chưa hoàn thành nhiệm vụ học tập, cần chăm chỉ và cẩn thận hơn.",
    "Em thao tác còn chậm, cần tập trung và kiên trì hơn trong giờ học.",
    "Em cần rèn luyện thêm tính tự giác và ý thức học tập.",
    "Em cần tập trung và kiên trì hơn trong giờ học để tiến bộ.",
    "Em chưa chú ý khi thực hành, cần được nhắc nhở và hỗ trợ thêm.",
    "Em làm việc chưa cẩn thận, cần cố gắng hơn trong giờ Công nghệ.",
    "Em cần chủ động hơn trong học tập và làm việc nhóm.",
    "Em chưa hoàn thành nhiệm vụ học tập, cần chăm chỉ và cẩn thận hơn.",
  ],
  yeu: [
    "Em cần cố gắng hơn trong học tập và rèn luyện kỹ năng thực hành.",
    "Em còn lúng túng khi thực hành, cần sự giúp đỡ của thầy cô và bạn bè.",
    "Em chưa hoàn thành nhiệm vụ học tập, cần chăm chỉ và cẩn thận hơn.",
    "Em thao tác còn chậm, cần tập trung và kiên trì hơn trong giờ học.",
    "Em cần rèn luyện thêm tính tự giác và ý thức học tập.",
    "Em cần tập trung và kiên trì hơn trong giờ học để tiến bộ.",
    "Em chưa chú ý khi thực hành, cần được nhắc nhở và hỗ trợ thêm.",
    "Em làm việc chưa cẩn thận, cần cố gắng hơn trong giờ Công nghệ.",
    "Em cần chủ động hơn trong học tập và làm việc nhóm.",
    "Em chưa hoàn thành nhiệm vụ học tập, cần chăm chỉ và cẩn thận hơn.",
  ]
};

// Cuối kỳ I hoặc cả năm
export const nhanXetCongNgheCuoiKy = {
  tot: {
    lyThuyet: [
      "Em học tích cực, hoàn thành tốt sản phẩm",
      "Em có ý thức học tập, chủ động trong lý thuyết",
      "Em tự tin trình bày ý tưởng, nắm chắc kiến thức",
      "Em luôn hoàn thành bài tập đúng hạn",
      "Em hợp tác tốt trong nhóm lý thuyết",
      "Em sáng tạo trong trình bày và áp dụng kiến thức",
      "Em chủ động đặt câu hỏi và thảo luận",
      "Em trình bày lý thuyết rõ ràng, dễ hiểu"
    ],
    thucHanh: [
      "thực hành đúng hướng dẫn, sản phẩm đạt yêu cầu.",
      "làm việc chăm chỉ, giữ gìn dụng cụ và an toàn.",
      "hứng thú trong giờ học, thực hiện đúng hướng dẫn.",
      "thao tác thành thạo, sản phẩm khoa học và logic.",
      "biết vận dụng kiến thức giải quyết tình huống.",
      "hoàn thành sản phẩm đúng quy trình, sáng tạo.",
      "sản phẩm đẹp mắt, áp dụng tốt kỹ thuật học.",
      "thực hành chính xác, chú ý từng bước."
    ]
  },

  kha: {
    lyThuyet: [
      "Em học tập tốt, hoàn thành nhiệm vụ",
      "Em cần mạnh dạn hơn khi trình bày ý tưởng",
      "Em nắm kiến thức cơ bản, cần luyện tập thêm",
      "Em có tiến bộ, cần phát huy tính chủ động",
      "Em trình bày bài học logic, nên sáng tạo hơn",
      "Em hoàn thành bài học cơ bản, cần cải thiện tự tin",
      "Em nên luyện tập thêm để nắm chắc kiến thức trọng tâm",
      "Em cần chủ động hơn khi làm bài và tham khảo thêm tài liệu",
      "Em học đúng nhưng cần tự tin hơn khi trình bày",
      "Em cần hỗ trợ để áp dụng kiến thức tốt hơn",
      "Em nên luyện tập thường xuyên để nắm chắc kiến thức"
    ],
    thucHanh: [
      "thực hành đúng hướng dẫn, cần rèn kỹ năng thêm",
      "làm việc cẩn thận, sản phẩm tốt nhưng cần cố gắng hơn",
      "sử dụng dụng cụ đúng cách, nên tập trung hơn",
      "hoàn thành sản phẩm nhưng cần cải thiện thẩm mỹ",
      "thao tác cơ bản tốt, nên nâng cao tốc độ và chính xác",
      "tham gia tích cực, cần chủ động hơn trong nhóm",
      "sản phẩm đầy đủ nhưng có thể thêm sáng tạo",
      "cần luyện tập để hoàn thiện kỹ năng",
      "thực hành đúng nhưng sản phẩm chưa thật đẹp, cần cải thiện",
      "còn lúng túng, nên nhờ thầy cô và bạn bè hỗ trợ",
      "thao tác chậm, cần luyện kỹ năng cơ bản thêm",
      "một số nhiệm vụ chưa hoàn thành, cần chú ý hơn",
      "làm việc vội vàng, cần rèn tính tỉ mỉ hơn",
      "cần luyện tập nâng cao kỹ năng để đạt chuẩn",
      "cần cải thiện thao tác, sản phẩm chưa đạt yêu cầu",
      "cần chú ý kỹ thuật để nâng cao chất lượng"
    ]
  },

  trungbinh: {
    lyThuyet: [
      "Em hoàn thành cơ bản, cần cố gắng hơn",
      "Em học còn chậm, cần tập trung",
      "Em nắm kiến thức trọng tâm, cần luyện tập thêm",
      "Em cần chủ động hơn khi làm bài",
      "Em học đúng nhưng thiếu tự tin",
      "Em cần hỗ trợ để hiểu và vận dụng kiến thức",
      "Em nên tham khảo thêm tài liệu",
      "Em nên luyện tập thường xuyên"
    ],
    thucHanh: [
      "cần cố gắng hơn trong thực hành.",
      "còn lúng túng, cần hỗ trợ thầy cô và bạn bè.",
      "chưa hoàn thành nhiệm vụ, cần chăm chỉ và cẩn thận.",
      "thao tác chưa chính xác, cần luyện tập nhiều hơn.",
      "thiếu chủ động, cần nhắc nhở và hướng dẫn thêm.",
      "chưa áp dụng kiến thức vào thực hành.",
      "cần luyện tập đều đặn để nâng cao kỹ năng.",
      "cần tham khảo hướng dẫn chi tiết để hoàn thành sản phẩm."
    ]
  },

  yeu: {
    lyThuyet: [
      "Em cần cố gắng hơn",
      "Em gặp khó khăn, cần hỗ trợ",
      "Em chưa hoàn thành yêu cầu, cần chăm chỉ",
      "Em nắm kiến thức yếu, cần kế hoạch học tập",
      "Em thiếu tự tin và chủ động",
      "Em cần kèm cặp sát sao để tiến bộ",
      "Em nên ôn luyện thường xuyên",
      "Em cần tập trung và chủ động tham gia"
    ],
    thucHanh: [
      "cần cố gắng hơn trong thực hành.",
      "còn lúng túng, cần hỗ trợ thầy cô và bạn bè.",
      "chưa hoàn thành nhiệm vụ, cần chăm chỉ và cẩn thận.",
      "thao tác chưa chính xác, cần luyện tập nhiều hơn.",
      "thiếu chủ động, cần nhắc nhở và hướng dẫn thêm.",
      "chưa áp dụng kiến thức vào thực hành.",
      "cần luyện tập đều đặn để nâng cao kỹ năng.",
      "cần tham khảo hướng dẫn chi tiết để hoàn thành sản phẩm."
    ]
  }
};







