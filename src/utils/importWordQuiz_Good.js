    ...
    elements.forEach((el) => {
      ...
      if (el.tagName === "TABLE") {
        ...
        if (colCount > 2) {
          ...
          // ❌ KHÔNG tạo câu image nữa
          const questionImage = images[0] || "";

          finalQuestions.push({
            id: `q_${Date.now()}_image_${index++}`,
           question: `<p>${escapeHTML(
      ...
      if (el.tagName === "P") {
        ...
        if (type === "choice")
          finalQuestions.push(parseChoice(block, index++, questionImage));
        else if (type === "sort")
          finalQuestions.push(parseSort(block, index++, questionImage));
        else if (type === "fillblank")
          finalQuestions.push(parseFillBlank(block, index++, questionImage));
      }
    });
